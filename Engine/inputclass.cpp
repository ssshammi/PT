...
bool InputClass::ReadKeyboard()
{
	...
	// Read the keyboard device.
		memcpy(m_keyboardPrevious,m_keyboardState,sizeof(unsigned char)*256);
...
bool InputClass::OnRelease(unsigned char ch) {
	if (!m_keyboardState[ch] && (m_keyboardState[ch] != m_keyboardPrevious[ch]))
	{
		return true;
	}
...
bool InputClass::OnPressed(unsigned char ch) {
	if (m_keyboardState[ch] && (m_keyboardState[ch] != m_keyboardPrevious[ch]))
	{
		return true;
	}
...
bool InputClass::IsSPressedOnce()
{
	// Do a bitwise and on the keyboard state to check if the key is currently being pressed.
	return OnPressed(DIK_S);
}

bool InputClass::IsVPressedOnce()
{
	// Do a bitwise and on the keyboard state to check if the key is currently being pressed.
...
bool InputClass::IsLPressedOnce()
{
	// Do a bitwise and on the keyboard state to check if the key is currently being pressed.
	return OnPressed(DIK_L);
}
...
bool InputClass::IsBPressedOnce()
{

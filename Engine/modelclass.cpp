...
bool ModelClass::LoadModelObj(char* filename) {
	...
	int pos = 2;
	string st;
	fout.open("../NamanEngine/data/ModelErrors.txt");
	fin.seekg(0, ios::beg);
	...
	return true;*/
	LoadFromMaya* loadMaya = new LoadFromMaya;
	char newName[256];
	int num = 0;

	loadMaya->LoadFromMayaNow(filename, *newName, num);
	bool result = LoadModel("../Engine/data/newModel.txt");
	if (!result)
		return false;
